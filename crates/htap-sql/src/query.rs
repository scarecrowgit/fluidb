...
impl TableSlot {
    ...
    pub fn alias(&self) -> &str {
        match self {
            Self::Base { alias, .. } | Self::Derived { alias, .. } => alias,
        }
    }
    ...
    pub fn columns(&self) -> &[ColumnDef] {
        match self {
            Self::Base { columns, .. } | Self::Derived { columns, .. } => columns,
        }
    }
...
pub struct BoundQuery {
    ...
    /// Uncorrelated subqueries appearing in expressions, indexed by the `index` fields of
