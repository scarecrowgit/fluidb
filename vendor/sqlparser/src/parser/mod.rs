...
impl<'a> Parser<'a> {
    ...
    pub fn parse_create_table(
        ...
        Ok(CreateTableBuilder::new(table_name)
            ...
            .mysql_partition_by(create_table_config.mysql_partition_by)
    ...
    fn parse_optional_create_table_config(
        ...
        let mysql_partition_by = if dialect_of!(self is MySqlDialect | GenericDialect) {
            self.maybe_parse_mysql_partition_by()?
        } else {
            None
        };
    ...
    /// Parse optional MySQL PARTITION BY clause.
    pub fn maybe_parse_mysql_partition_by(
        &mut self,
    ) -> Result<Option<MysqlPartitionBy>, ParserError> {
        if !self.parse_keywords(&[Keyword::PARTITION, Keyword::BY]) {
            return Ok(None);
        }

        if self.parse_keyword(Keyword::RANGE) {
            let (columns, expr) = if self.parse_keyword(Keyword::COLUMNS) {
                self.expect_token(&Token::LParen)?;
                let cols = self.parse_comma_separated(Parser::parse_identifier)?;
                self.expect_token(&Token::RParen)?;
                (Some(cols), None)
            ...
            self.expect_token(&Token::LParen)?;
            let partitions = self.parse_comma_separated(Parser::parse_mysql_partition_def)?;
            self.expect_token(&Token::RParen)?;

            Ok(Some(MysqlPartitionBy::Range {
                columns,
                expr,
                partitions,
            }))
            ...
            Ok(Some(MysqlPartitionBy::List {
                columns,
                expr,
                partitions,
            }))
        } else {
            self.expected("RANGE or LIST after PARTITION BY", self.peek_token())
        }
    }

    /// Parse a single MySQL partition definition: `PARTITION name VALUES LESS THAN (...)` or `PARTITION name VALUES IN (...)`.
    pub fn parse_mysql_partition_def(&mut self) -> Result<MysqlPartitionDef, ParserError> {
        ...
        let values = if self.parse_keywords(&[Keyword::LESS, Keyword::THAN]) {
            if self.parse_keyword(Keyword::MAXVALUE) {
                MysqlPartitionValues::LessThan(MysqlLessThanBound::MaxValue)
            } else {
                self.expect_token(&Token::LParen)?;
                let bound = if self.parse_keyword(Keyword::MAXVALUE) {
                    MysqlLessThanBound::MaxValue
                } else {
                    MysqlLessThanBound::Expr(self.parse_expr()?)
                };
                self.expect_token(&Token::RParen)?;
                MysqlPartitionValues::LessThan(bound)
            }
        } else if self.parse_keyword(Keyword::IN) {
            self.expect_token(&Token::LParen)?;
            let exprs = self.parse_comma_separated(Parser::parse_expr)?;
            self.expect_token(&Token::RParen)?;
            MysqlPartitionValues::In(exprs)
        } else {
            return self.expected("LESS THAN or IN after VALUES", self.peek_token());
        };

        Ok(MysqlPartitionDef { name, values })
    }
