...
pub struct CreateTable {
    ...
    /// MySQL `PARTITION BY` partitioning specification.
    /// <https://dev.mysql.com/doc/refman/8.4/en/create-table.html>
    pub mysql_partition_by: Option<MysqlPartitionBy>,
}
...
impl fmt::Display for CreateTable {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if let Some(mysql_partition_by) = &self.mysql_partition_by {
            write!(f, " {mysql_partition_by}")?;
        }
...
/// MySQL `PARTITION BY` partitioning specification for `CREATE TABLE`.
///
/// See <https://dev.mysql.com/doc/refman/8.4/en/create-table.html>
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub enum MysqlPartitionBy {
    /// `PARTITION BY RANGE (expr)` or `PARTITION BY RANGE COLUMNS (col_list)`
    Range {
        /// Column list if `COLUMNS` syntax is used, otherwise `None`.
        columns: Option<Vec<Ident>>,
        /// Single expression if non-COLUMNS syntax is used, otherwise `None`.
        expr: Option<Expr>,
        /// Partition definitions.
        partitions: Vec<MysqlPartitionDef>,
    },
    /// `PARTITION BY LIST (expr)` or `PARTITION BY LIST COLUMNS (col_list)`
    List {
        /// Column list if `COLUMNS` syntax is used, otherwise `None`.
        columns: Option<Vec<Ident>>,
        /// Single expression if non-COLUMNS syntax is used, otherwise `None`.
...
impl fmt::Display for MysqlPartitionBy {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            MysqlPartitionBy::Range {
                ...
                write!(f, "PARTITION BY RANGE ")?;
                if let Some(cols) = columns {
                    write!(f, "COLUMNS ({}) ", display_comma_separated(cols))?;
                } else if let Some(e) = expr {
                    write!(f, "({e}) ")?;
                }
                write!(f, "({})", display_comma_separated(partitions))
            }
...
/// A partition definition in MySQL `PARTITION BY RANGE/LIST (...)`.
///
/// Example: `PARTITION p0 VALUES LESS THAN (10)` or `PARTITION p1 VALUES IN (1, 2)`
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub struct MysqlPartitionDef {
    /// Partition name.
    pub name: Ident,
    /// Partition boundary or values clause.
    pub values: MysqlPartitionValues,
}

impl fmt::Display for MysqlPartitionDef {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "PARTITION {} {}", self.name, self.values)
    }
}

/// Boundary/values clause in a MySQL partition definition.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub enum MysqlPartitionValues {
    /// `VALUES LESS THAN (expr | MAXVALUE)`
    LessThan(MysqlLessThanBound),
    /// `VALUES IN (expr, ...)`
    In(Vec<Expr>),
}

impl fmt::Display for MysqlPartitionValues {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            MysqlPartitionValues::LessThan(b) => write!(f, "VALUES LESS THAN ({b})"),
            MysqlPartitionValues::In(items) => {
                write!(f, "VALUES IN ({})", display_comma_separated(items))
            }
        }
    }
}

/// Bound in `VALUES LESS THAN (...)`.
#[derive(Debug, Clone, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[cfg_attr(feature = "serde", derive(Serialize, Deserialize))]
#[cfg_attr(feature = "visitor", derive(Visit, VisitMut))]
pub enum MysqlLessThanBound {
    /// An explicit value expression.
    Expr(Expr),
    /// The MySQL `MAXVALUE` keyword.
    MaxValue,
}

impl fmt::Display for MysqlLessThanBound {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            MysqlLessThanBound::Expr(e) => write!(f, "{e}"),
            MysqlLessThanBound::MaxValue => write!(f, "MAXVALUE"),
        }
    }
}
