...
- **Bulk CSV/JSON-lines Import Rounds a `DECIMAL` Value Beyond the Column's Declared Scale, Rather Than Rejecting It (Phase 17):** Importing a decimal value with more fractional digits than its target column's declared scale (e.g. `1.235` into a `DECIMAL(_,2)` column) rounds it half-away-from-zero — the same rule an ordinary `INSERT`/`UPDATE` assignment uses — instead of returning an error. This keeps the bulk and interactive paths consistent, but it means loading third-party data through CSV/JSON-lines import can silently alter values; check a source's decimal scale against the target column before a bulk load if exactness matters. A value that overflows the column's declared *precision* (as opposed to just its scale) is still rejected with `HtapError::InvalidArgument`, never rounded away. Separately, a decimal value in JSON-lines is always a JSON *string* (e.g. `"amount":"1.2345"`), never a bare JSON number — a bare number is rejected outright, specifically to keep a float round trip from mangling the value before the engine's own exact decimal-text parser ever sees it; a JSON source that emits bare numbers for a decimal column will need a pre-processing step before import. Covered by `crates/htap-movement/tests/decimal_codec.rs` (`test_decimal_import_rounds_extra_fractional_digits_half_away_from_zero`, `test_decimal_precision_overflow_is_rejected_by_csv_and_jsonl`, `test_jsonl_decimal_roundtrip_cases_emit_strings`, `test_jsonl_decimal_number_is_rejected_with_column_name`).
