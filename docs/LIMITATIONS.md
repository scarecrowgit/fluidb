...
- **MySQL wire protocol and `htapd` daemon (implemented, Phase 8):** A hand-written, synchronous MySQL
  ...
  - **No sessions or transaction control over the wire:** Same limitation as the in-process engine — no
    `BEGIN`/`COMMIT`/`ROLLBACK`; every statement auto-commits. `SET ...` statements are accepted as no-ops.
