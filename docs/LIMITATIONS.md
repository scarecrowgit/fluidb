...
- No full SQL analytics. A network MySQL daemon (`htapd`/`htap-wire`) is now implemented (Phase 8) with a
  ...
  64-bit values above `i64::MAX`, arbitrary-precision `DECIMAL` over the wire — the engine's own `DECIMAL`
  type, including a `DECIMAL` result column over both the text and binary protocols, is implemented as of
  Phase 17, but only up to the engine's own 18-digit bound, see "`DECIMAL` type (Phase 17) scope and deferred
  features" above — `TIME`-typed parameters, best-effort `PREPARE` metadata).
...
- **Sessions and explicit transaction control (implemented, Phase 10):** `htap-server::session::Session`
  ...
  - **Prepared statements (Phase 11) go through the same session and quarantine gate as `COM_QUERY`:**
    ...
    values, arbitrary-precision `DECIMAL` over the wire (a `DECIMAL` parameter and a `DECIMAL` result column
    both work, bounded at the engine's own 18-digit maximum — see "`DECIMAL` type (Phase 17) scope and
    deferred features" above), `TIME` parameters, best-effort `PREPARE` metadata).
...
- **`DECIMAL` type (Phase 17) scope and deferred features:** `DataType::Decimal { precision, scale }` /
  `Value::Decimal` — a fixed-point value stored as a signed 64-bit integer scaled by a power of ten, maximum
  18 digits, exact round-half-away-from-zero arithmetic, and derived *result* precision/scale clamped to the
  18-digit maximum rather than the query being rejected (a *value* exceeding its own declared precision is
  still a hard error; nothing ever silently becomes a float — see `docs/DECISIONS.md`'s ADR-026 (Amendment 4),
  `docs/ARCHITECTURE.md`'s "Derived `DECIMAL` precision and scale rules" for the exact per-operator reference,
  and `docs/PROGRESS.md`'s Phase 17 row). Both A6a (query layer: literals,
  `CAST`, arithmetic, comparisons, ordering/hashing, `SUM`/`AVG`/`MIN`/`MAX`/`COUNT(DISTINCT)` grouped and
  windowed) and A6b (persistence: `CREATE TABLE`/`INSERT`/literal coercion, the columnar segment format
  (`HTAPCOL1` v1 -> v2, decimal pushdown filtering included), the composite-key codec, the rowstore memtable's
  size estimator, catalog recovery, the movement crate's CSV/JSON-lines codec, and the MySQL wire protocol) are
  complete — see `docs/PROGRESS.md`'s Phase 17 row for the full task-by-task test evidence. Disclosed gaps:
  - **Bulk import rounds a value beyond the column's declared scale rather than rejecting it.** The CSV and
    JSON-lines import path (`crates/htap-movement/src/codec.rs`) rounds a decimal value carrying more
    fractional digits than the target column's scale half-away-from-zero, the same rule an ordinary
    `INSERT`/`UPDATE` assignment uses — a deliberate choice to keep the bulk and interactive paths consistent,
    but it means loading third-party data through `COPY`/import can silently alter values; see
    `docs/OPERATIONS.md` section 2 for the operator-facing statement of this. A value that overflows the
    declared *precision* (as opposed to just the scale) is still a hard `HtapError::InvalidArgument`, never
    rounded away. Covered by
    `crates/htap-movement/tests/decimal_codec.rs::{test_decimal_import_rounds_extra_fractional_digits_half_away_from_zero, test_decimal_precision_overflow_is_rejected_by_csv_and_jsonl}`.
  - **A decimal is a JSON string, not a JSON number, in JSON-lines.** `decode_record`/`encode_record` always
    emit and expect a decimal column as a quoted string (e.g. `"amount":"1.2345"`); a bare JSON number for a
    decimal column is rejected outright, specifically to keep a float round trip from mangling the value
    before this project's own exact decimal-text parser ever sees it. This is an interoperability trade-off —
    most JSON-emitting tools write bare numbers, not strings, for numeric columns — so a third-party JSON-lines
    source will need a pre-processing step. Covered by
    `crates/htap-movement/tests/decimal_codec.rs::{test_jsonl_decimal_roundtrip_cases_emit_strings, test_jsonl_decimal_number_is_rejected_with_column_name}`.
  - **`DECIMAL` over the wire is bounded at 18 digits, not arbitrary precision.** See "No exact `DECIMAL`" /
    "`DECIMAL` over the wire is bounded at the engine's own 18 digits" above and in "Prepared statements and
    binary protocol scope and deferred features" below — a `DECIMAL` result column and parameter both work,
    up to the engine's own bound, not beyond it.
...
- **`DECIMAL` over the wire is bounded at the engine's own 18 digits, not arbitrary precision (Phase 17):**
  `NEWDECIMAL`/`DECIMAL` parameters are decoded and substituted as exact numeric literal text
  (`htap_sql::ParamLiteral::NumericText`), never through a lossy `f64`/`i64` round trip — the wire-to-AST step
  itself loses no precision. From there the binder treats it exactly like the same numeric literal typed
  directly into SQL text, coercing it into the target column's own declared type — as of Phase 17 that
  includes a `DECIMAL(p, s)` target, which binds exactly up to the target's own precision (`p` up to 18
  digits); a target `Int64`/`Float64` column coerces as before, and precision beyond what the target type can
  hold is still lost at that step, not before it. A `DECIMAL` *result* column is also implemented over both
  the text and binary result-row protocols (`crates/htap-wire/src/{result_codec,binary_codec}.rs`), not only
  as a bound parameter — see "`DECIMAL` type (Phase 17) scope and deferred features" above for the test
  evidence. What remains unsupported is arbitrary-precision decimal arithmetic: the engine's own `DECIMAL` is
  bounded at 18 digits everywhere, on every path, not just over the wire.
