...
- **Partition Execution & Multi-Partition Routing Scope:**
  ...
  - Analytic `SELECT` scans all partitions of the table at a single visible snapshot and merges rows for global or grouped evaluation (`test_partitioned_olap_across_partitions_and_empty_aggregate`). Partition pruning, parallel multi-core execution, and global cross-partition ordering are not claimed or implemented.
